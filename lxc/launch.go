...
func (c *launchCmd) run(config *lxd.Config, args []string) error {
	...
	if name == "" {
		if resp.Resources == nil {
			return fmt.Errorf(gettext.Gettext("didn't get any affected image, ccontainer or snapshot from server"))
		}
		...
		if count != 2 {
			return fmt.Errorf(gettext.Gettext("bad number of things scanned from image, ccontainer or snapshot"))
		}
